...
#' @examples
#' dataExtract()
dataExtract <- function(dbconnection, cdmDatabaseSchema, workDatabaseSchema,
                        cohortid=100, agegroup=3, gender=8507,type='history')
{
  ...
  writeSql(sql, file.path(getwd(),"rendered_count_extractor.sql"))
  ...
  insertTable(conn2, paste(workDatabaseSchema,'JMR_covariates',sep='.'), definitions, dropTableIfExists = TRUE,
  ...
  dbDisconnect(conn2)
    ...
    sql <- translateSql(sql = sql, sourceDialect = "sql server", targetDialect = sqlType)$sql
    writeSql(sql, file.path(getwd(), paste("rendered_",type,"_extraction.sql")))
    ##clust.data <- as.ffdf(querySql(conn, sql))   - caused crashing issues
    clust.data <- querySql(conn, sql)
...
.checkdata <- function(dbconnection, cdmDatabaseSchema, workDatabaseSchema,
